...
public class RuleAlertPublisher {
    ...
    private final long rearmMinutes;
    private Clock clock;
    ...
    public RuleAlertPublisher(AlertRepository alertRepository,
                              ...
                              @Value("${alert.suppression.rearm-minutes:20}") long rearmMinutes) {
        this(alertRepository, ledgerRepository, stateStore, rearmMinutes, Clock.systemUTC());
    }
    ...
    public RuleAlertPublisher(AlertRepository alertRepository,
        ...
        this.rearmMinutes = rearmMinutes;
    ...
    private boolean isSuppressed(String alertId) {
        Optional<String> val = stateStore.get("suppressed:" + alertId);
        if (val.isEmpty()) {
            return false;
        }
        try {
            long expiryMillis = Long.parseLong(val.get());
            if (clock.instant().toEpochMilli() < expiryMillis) {
                return true;
            }
    ...
    public void suppressLevel(String alertId) {
        if (alertId != null && isLevelAlertId(alertId)) {
            Instant expiresAt = clock.instant().plus(Duration.ofMinutes(rearmMinutes));
            stateStore.put("suppressed:" + alertId, String.valueOf(expiresAt.toEpochMilli()));
        }
    }
