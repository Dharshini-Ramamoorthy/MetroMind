...
Brand block: logo, one-line description, "Restricted Operator Domain" pill with lock icon.
"Line Infrastructure" telemetry list (label + status value rows, dashed separators).
"Relay Telemetry" list (same row style).
...
Changing weather / surge / incident instantly recalculates and updates, with no page reload:
...
The 4 scenario preset buttons are shortcuts that set all three inputs at once and immediately trigger the same recalculation; manually adjusting any single control deselects the preset buttons.
Weather makes headways worse (Clear → Wet Rails +15% → Heavy Monsoon +40%); higher passenger surge shortens headways (more frequent trains); the incident toggle adds a fixed safety delay; all values are clamped to a safe minimum floor.